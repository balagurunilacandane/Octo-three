...
export function generateLayout(b: WorldBundle): WorldLayout {
  ...
  const plots: PlotLayout[] = teams.map((team, index) => {
    ...
    obstacles.push({ c: buildingPos, hx: fp.w / 2, hz: fp.d / 2, rot: CONTENT_ROT, height: fp.h })
    ...
    return {
      ...
      gate,
      dock,
      emitter: [buildingPos[0], fp.h + 0.4, buildingPos[1]],
    }
  })
  ...
  return { key: layoutKey(b), half, plots, plotById, brain, obstacles, heightAt }
}
